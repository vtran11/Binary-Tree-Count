...
void QuadraticPtrHashTable<HashedObj>::rehash( )
{
    ...
    for( int j = 0; j < array.size( ); j++ )   //new array
        array[ j ]= NULL;  //empty table
    
    // Copy table over
    currentSize = 0;
    for( int i = 0; i < oldArray.size( ); i++ )   //get all elements from old table
        if( oldArray[ i ] != NULL && oldArray[i] != & ITEM_NOT_FOUND )   //if the position is fit
            insert(*oldArray[ i ]); //insert that element into new table
}
...
int QuadraticPtrHashTable<HashedObj>::findPos( const HashedObj & x ) const
{
    ...
    /* 3*/      while( array[ currentPos ] != NULL && array [currentPos] != & ITEM_NOT_FOUND &&
                      *(array[ currentPos ])!= x )
    {
...
{
    return array[ currentPos ] != NULL && array[currentPos] != &ITEM_NOT_FOUND;  //current position doesn't contain number
}
